...
class RayPPOTrainer(object):
    ...
    def _relabel_prompt(self, orig_prompt, prediction) -> str:
        pattern = r"(create an equation that equals )\d+(\.)"

        def repl_func(match):
            return f"{match.group(1)}{prediction}{match.group(2)}"

        rep = re.sub(pattern, repl_func, orig_prompt, flags=re.IGNORECASE)
        return rep 
    ...
    def _apply_hindsight_relabeling(self, batch: DataProto):
        ...
        for i in range(len(batch)):
            ...
            # If a valid equation / solution is not generated by the LLM, then no hindsight relabeling occurs
            equation_str = extract_solution(solution_str=sequences_str)
            if not equation_str:
                continue 
            available_numbers = batch.non_tensor_batch['reward_model'][i]['ground_truth']["numbers"]
            if not validate_equation(equation_str, available_numbers):
                continue 
            try: 
                prediction = evaluate_equation(equation_str)
                if not prediction:
                    continue 
            except:
                continue 
            ...
            assert attention_all_equal, "Attention masks are not equivalent"
            assert position_id_all_equal, "Position ids are not equivalent" 
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    self._apply_hindsight_relabeling(batch)
